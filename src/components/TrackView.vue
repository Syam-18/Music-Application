...
const toggleLike = async (track) => {
  ...
  if (!user) {
    triggerToast('Please log in to like songs')
    return
  }

  try {
    const isLiked = likedTracks.value.some((likedTrack) => likedTrack.id === track.id)
    ...
    // Refresh liked tracks from Firestore
    likedTracks.value = await getLikedSongs()
  } catch (err) {
